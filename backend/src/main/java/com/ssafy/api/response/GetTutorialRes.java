...
public class GetTutorialRes {
    ...
    public static GetTutorialRes fromEntity(UserTutorialProgress tutorialProgress) {
        return new GetTutorialRes(
            tutorialProgress.getId(),
            tutorialProgress.getTutorialType().getId(),  // tutorialType의 id만 가져오기
            tutorialProgress.getIsCompleted(),
            tutorialProgress.getCompletedAt()
        );
    }
}