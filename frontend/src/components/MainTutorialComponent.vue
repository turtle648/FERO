  ...
  <div class="img-container z-50 fixed inset-0 flex items-center justify-center bg-black bg-opacity-50" @click="nextStep">
    ...
    <section class="message-list flex items-center absolute top-3/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
      ...
      <div class="desc-container nes-balloon from-left ml-4 w-[39vh] h-[13vh] -translate-y-1 flex justify-center items-center">
        ...
        <span v-if="currentStep === 3" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          상단의
          <span class="text-red-500">유저정보</span>
          를 클릭하면 스탯을 확인할 수 있습니다
        </span>
        <span v-if="currentStep === 4" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          메인의 중앙에 있는
          <span class="text-red-500">캐릭터를 클릭</span>
          하면
        </span>
        <span v-if="currentStep === 5" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">캐릭터의 얼굴, 옷, 헤어스타일을 바꿀 수 있습니다</span>
        <span v-if="currentStep === 6" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          이제
          <span class="text-red-500">하단 아이콘</span>
          에 대해 설명 드리겠습니다
        </span>
        <span v-if="currentStep === 7" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          가장 왼쪽의
          <span class="text-red-500">Calendar</span>
          를 클릭하면
        </span>
        ...
        <span v-if="currentStep === 13" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          ...
          <span class="text-red-500">랭크모드</span>
          가 있습니다
        </span>
        <span v-if="currentStep === 14" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          <span class="text-red-500">Quest 버튼</span>
          은 매일 퀘스트를 확인할 수 있습니다
        </span>
        <span v-if="currentStep === 15" class="font-dgm sm:text-xs md:text-lg lg:text-xl xl:text-2xl">
          그리고 마지막으로
          <span class="text-red-500">Setting 버튼</span>
          에서는 회원탈퇴와
        </span>
