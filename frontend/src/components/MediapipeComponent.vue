<template>
  <div class="container">
    <!-- 중앙 영역 -->
    <div
      class="start-container flex absolute inset-0 items-center justify-center h-screen z-10"
    >
      <div v-if="countdown > 0" class="countdown text-5xl z-10">
        {{ countdown }}
      </div>
      <div v-else-if="showStartText" class="start-text text-5xl z-10">
        START!
      </div>
    </div>
...
const onResults = (results) => {
  ...
  if (results.poseLandmarks) {
    ...
    if (nose && leftEar && rightEar && leftShoulder && rightShoulder) {
      ...
      // ✅ **2. 블러 처리 후 이모지 그리기**
      const earDistance =
        Math.abs(leftEar.x - rightEar.x) * canvasElement.value.width; // 귀 간 거리 계산
