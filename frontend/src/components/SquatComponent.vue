<template>
  <div class="squat-container w-screen h-screen">
    ...
    <div class="text-container absolute top-4 right-4">
      <div class="count text-yellow-500 text-4xl font-dgm">스쿼트 횟수: {{ count }}</div>
      <div v-if="showGreat" class="great-message text-red text-3xl">Great!</div>
    </div>

    <div v-if="showErrorModal" class="landmark-error-modal absolute inset-0 flex items-center justify-center text-yellow-500 text-4xl z-20">전신이 나오도록 위치를 수정해주세요</div>
    <button v-if="isTutorialMode" @click="setCountToThree" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 absolute bottom-4 left-4">Set Count to 3</button>
  </div>
</template>
